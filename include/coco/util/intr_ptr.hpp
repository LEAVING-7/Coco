...
struct ControlBlock {
  ...
  explicit ControlBlock(Us&&... us) noexcept(noexcept(T{std::declval<Us>()...})) : mRc(1)
  {
...
private:
  ...
  {
    if (mData != nullptr) {
      mData->mRc.fetch_add(1, std::memory_order_relaxed);
    }
  }

  auto release() noexcept -> void
  {
    if (mData != nullptr && mData->mRc.fetch_sub(1, std::memory_order_release) == 1) {
